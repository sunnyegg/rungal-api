...
module.exports = {
  ...
  getProducts: lim => {
    return new Promise((resolve, reject) => {
      conn.query(
        `SELECT a.id, a.name, a.description, a.image, b.name as category, a.price, a.quantity, a.date_added, a.date_updated FROM product a, category b WHERE a.category=b.id AND a.name LIKE ? ORDER BY a.${lim.b} ${lim.e} LIMIT ?,?`,
        [lim.a, lim.c, lim.d],
        (err, result) => {
          if (!err) {
            resolve(result);
  ...
  addQuantityProduct: (qty, id) => {
    return new Promise((resolve, reject) => {
      conn.query(
        "UPDATE product SET quantity = quantity + ? WHERE ?",
        [qty, id],
        (err, result) => {
          if (!err) {
            resolve(result);
  ...
  reduceQuantityProduct: (qty, id) => {
    return new Promise((resolve, reject) => {
      conn.query("SELECT quantity FROM product WHERE ?", id, (err, result) => {
        if (result.length > 0) {
          const quantity = parseInt(result[0].quantity, 10) - parseInt(qty, 10);
          if (quantity > 0) {
            conn.query(
              "UPDATE product SET quantity = ? WHERE ?",
              [quantity, id],
              err => {
                if (!err) {
                  resolve(result);
  ...
  sortProduct: parameter => {
    return new Promise((resolve, reject) => {
      conn.query(
        `SELECT a.id, a.name, a.description, a.image, b.name AS category, a.price, a.quantity, a.date_added, a.date_updated FROM product a, category b WHERE a.category=b.id ORDER BY ${parameter}`,
        (err, result) => {
          if (!err) {
            resolve(result);
          } else {
